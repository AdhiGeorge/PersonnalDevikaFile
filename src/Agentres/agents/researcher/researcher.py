...
Your response must be a valid JSON object with the following structure:
{
    ...
    def research(self, query: str, plan: str) -> str:
        """Research the given query and plan"""
        self.logger.info(f"Starting research phase for query: {query}")
        ...
        for result in search_results:
            try:
                # Scrape the content
                content = self.browser.scrape(result['url'])
                
                # Log the scraped data
                self.logger.add_scraped_data(
                    url=result['url'],
                    content=content,
                    metadata={
                        'title': result.get('title', ''),
                ...
                self.knowledge_base.add_document(content, metadata={'url': result['url']})
                
            except Exception as e:
                self.logger.error(f"Error processing search result {result['url']}: {str(e)}")
        
        # Synthesize the research
        prompt = self.prompt_manager.get_prompt("researcher", query, plan)
        synthesis = self.llm.generate(prompt)
        
        # Log the synthesis
        self.logger.set_researcher_output(synthesis)
        self.logger.set_synthesized_data({
            'query': query,
            'plan': plan,
            'synthesis': synthesis,
